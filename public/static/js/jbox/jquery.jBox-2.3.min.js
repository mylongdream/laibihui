(function(a) {
	a.jBox = function(b, c) {
		...
		j.push('<div id="jbox-temp" class="jbox-temp" style="width:0px;height:0px;background-color:#ff3300;position:absolute;z-index:984;display:none;"></div>');
		if (c.draggable) {
			j.push('<div id="jbox-drag" class="jbox-drag" style="position:absolute;z-index:984;display:none;"></div>')
		};
		...
		if (g) {
			...
			j.push('<div class="jbox-title' + (c.showIcon == true ? ' jbox-title-icon' : (c.showIcon == false ? '' : ' ' + c.showIcon)) + '" style="float:left; width:' + i + '; padding-left:' + (c.showIcon ? 0x12 : 0x5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;">' + (c.title == '' ? '&nbsp;' : c.title) + '</div>');
			j.push('</div>')
		};
		j.push('<a class="jbox-close" title="' + a.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).removeClass(\'jbox-close-hover\');" style="position:absolute; display:block; cursor:pointer;' + (c.showClose ? '' : 'display:none;') + '"></a>');
		...
		a.each(b, function(N, O) {
			...
			r.push('<div style="min-width:30px;width:' + (typeof c.width == 'number' ? c.width + 'px' : 'auto') + '; height:' + Q + ';">' + R + '<div id="jbox-content" class="jbox-content" style="height:' + Q + ';overflow:hidden;overflow-y:auto;">' + O.content + '</div></div>');
		...
		var B = function() {
				if (c.isMessager) {
					...
					o.css({
						position: "absolute",
						top: (l.height() - o.outerHeight()) / 0x2 + (c.isTip ? l.scrollTop() : 0x0),
	...
	a.jBox.prompt = function(b, c, d, e) {
		...
		var g = e.icon == 'none' ? 'height:auto;' : 'min-height:50px;' + (('undefined' == typeof(document.body.style.maxHeight)) ? 'height:auto !important;height:100%;_height:50px;' : 'height:auto;');
		var h = [];
		h.push('html:');
		h.push('<div style="margin:10px;' + g + 'padding-left:' + (e.icon == 'none' ? 0x0 : 0x28) + 'px;text-align:left;position:relative">');
		h.push('<span class="jbox-icon jbox-icon-' + e.icon + '" style="position:absolute; top:0px;left:0px; width:32px; height:32px;"></span>');
		h.push(e.content);
		h.push('</div>');
		e.content = h.join('');
		a.jBox(e.content, e)
	...
	a.jBox.tip = function(b, c, d) {
		...
		e.push('html:');
		if(d.content){
            e.push('<div style="min-height:18px;height:auto;line-height:18px;margin:10px;padding-left:30px;padding-top:0px;text-align:left;">');
		}else{
            e.push('<div style="height:38px;width:38px;">');
		}
		e.push('<span class="jbox-icon jbox-icon-' + d.icon + '" style="position:absolute;top:' + (0x3 + d.border) + 'px;left:' + (0x3 + d.border) + 'px; width:32px; height:32px;"></span>');
		e.push(d.content);
		e.push('</div>');
		d.content = e.join('');
	...
	a.jBox.messager = function(b, c, d, e) {
		...
		var h = e.icon == 'none' ? 'height:auto;' : 'min-height:50px;' + (('undefined' == typeof(document.body.style.maxHeight)) ? 'height:auto !important;height:100%;_height:50px;' : 'height:auto;');
		var i = [];
		i.push('html:');
