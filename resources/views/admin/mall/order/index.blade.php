...
@section('content')
	<div class="itemnav">
		<div class="title"><h3>{{ trans('admin.mall.order') }}</h3></div>
	</div>
	<form id="schform" name="schform" class="formsearch" method="get" action="{{ route('admin.mall.order.index') }}">
	<div class="tbsearch">
		<dl>
			<dd>
	...
	<form id="cpform" name="cpform" class="ajaxform" method="post" action="{{ route('admin.mall.order.batch') }}">
	{!! csrf_field() !!}
	<input type="hidden" id="operate" name="operate" value="" />
	<div class="tblist">
		<div class="tbhead cl">
			<div class="z"><h3>{{ trans('admin.mall.order.list') }}</h3></div>
		</div>
		<table>
			<tr>
				<th width="24"><input class="checkall" type="checkbox"></th>
				<th width="100">{{ trans('admin.mall.order.user') }}</th>
				<th width="160">{{ trans('admin.mall.order.order_sn') }}</th>
				<th width="240">{{ trans('admin.mall.order.consignee') }}</th>
				<th>{{ trans('admin.mall.order.remark') }}</th>
				<th width="60">{{ trans('admin.mall.order.status') }}</th>
				<th width="120">{{ trans('admin.mall.order.created_at') }}</th>
			...
			<tr>
				...
				<td>{{ trans('admin.mall.order.status_'.$value->order_status.$value->shipping_status.$value->pay_status) }}</td>
				<td>{{ $value->created_at->format('Y-m-d H:i') }}</td>
				<td>
					<a href="{{ route('admin.mall.order.show',$value->id) }}" class="" title="{{ trans('admin.view') }}">{{ trans('admin.view') }}</a>
					<a href="{{ route('admin.mall.order.destroy',$value->id) }}" class="mlm delbtn">{{ trans('admin.destroy') }}</a>
				</td>
			</tr>
			@endforeach
		</table>
