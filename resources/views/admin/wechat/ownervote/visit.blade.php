...
@section('content')
	<div class="itemnav">
		...
		<ul class="tab">
			...
			<li class="current"><a href="{{ route('admin.wechat.ownervote.visit') }}"><span>访问记录</span></a></li>
			<li><a href="{{ route('admin.wechat.ownervote.share') }}"><span>分享记录</span></a></li>
		</ul>
	</div>
	<form id="schform" name="schform" class="formsearch" method="get" action="{{ route('admin.wechat.ownervote.visit') }}">
	<div class="tbsearch">
		<dl>
			<dt>用户昵称</dt>
	...
	<form id="cpform" name="cpform" class="ajaxform" method="post" action="{{ route('admin.wechat.ownervote.visit') }}">
	{!! csrf_field() !!}
	...
	<div class="pgs cl">
		...
		<div class="page y">
			{!! $visitlist->appends(['nickname' => request('nickname')])->appends(['openid' => request('openid')])->links() !!}
		</div>
    </div>
	@endif
	</form>
