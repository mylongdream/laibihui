...
@section('content')
	...
	<form class="ajaxform" enctype="multipart/form-data" method="post" action="{{ route('admin.brand.shop.update', $shop->id) }}">
		{!! method_field('PUT') !!}
		{!! csrf_field() !!}
		<div class="tbedit">
			<div class="tbhead cl">
			...
			<table>
					...
					<td>
						<select class="select" name="catid">
							...
							@foreach ($categorylist as $scategory)
								<option value="{{ $scategory->id }}" {!! $shop->catid == $scategory->id ? 'selected="selected"' : '' !!}>{{ str_repeat('->',$scategory->count-1) }}{{ $scategory->name }}</option>
							@endforeach
						</select>
					</td>
				</tr>
					...
					<td>
                        ...
                        <div class="uploadbox">
                            <ul>
                                @if ($shop->banner)
                                    <li class="trigger-hover">
                                        <img src="{{ uploadImage($shop->banner) }}" width="595" height="60">
                                        <input name="banner" value="{{ $shop->banner }}" type="hidden">
                                        <div class="handle"><span class="setdel">删除</span></div>
                                    </li>
                                @endif
                            </ul>
				...
				<tr>
					...
					<td><input class="txt" type="text" size="50" value="{{ $shop->openhours }}" name="openhours"></td>
				</tr>
				<tr>
					<td align="right">{{ trans('admin.brand.shop.address') }}</td>
					<td><textarea class="textarea" name="address" cols="60" rows="4">{{ $shop->address }}</textarea></td>
				</tr>
				<tr>
					<td align="right">{{ trans('admin.brand.shop.mappoint') }}</td>
					<td><input class="txt" type="text" size="15" value="{{ $shop->maplng }}" name="maplng" id="maplng"> X <input class="txt" type="text" size="15" value="{{ $shop->maplat }}" name="maplat" id="maplat">
						<a href="javascript:;" class="clickbtn mlm" id="mapmark">点击标注</a>
						<a href="{{ route('admin.brand.shop.nearby', ['id' => $shop->id]) }}" class="clickbtn mlm openwindow" title="附近店铺">附近店铺</a>
					</td>
				</tr>
				<tr>
					<td align="right">{{ trans('admin.brand.shop.phone') }}</td>
					<td><input class="txt" type="text" size="50" value="{{ $shop->phone }}" name="phone"></td>
				</tr>
				<tr>
					<td align="right">{{ trans('admin.brand.shop.mobile') }}</td>
					<td><input class="txt" type="text" size="50" value="{{ $shop->mobile }}" name="mobile"><span class="tdtip">用于接收短信通知，前台不显示</span></td>
				</tr>
                <tr>
                    <td align="right">{{ trans('admin.brand.shop.discount') }}</td>
                    <td><input class="txt" type="text" size="20" value="{{ $shop->discount }}" name="discount"> 折</td>
                </tr>
				<tr>
					<td align="right">{{ trans('admin.brand.shop.indiscount') }}</td>
					<td><input class="txt" type="text" size="20" value="{{ $shop->indiscount }}" name="indiscount"> 折<span class="tdtip">公司与商家签订的合同折扣，前台不显示</span></td>
				</tr>
				<tr>
					<td align="right">{{ trans('admin.brand.shop.expiry') }}</td>
					<td><input id="starttime" class="txt" type="text" size="20" value="{{ $shop->started_at ? $shop->started_at->format('Y-m-d H:i') : '' }}" name="started_at"> 至 <input id="endtime" class="txt" type="text" size="20" value="{{ $shop->ended_at ? $shop->ended_at->format('Y-m-d H:i') : '' }}" name="ended_at"></td>
				</tr>
				<tr>
					<td align="right">{{ trans('admin.brand.shop.viewnum') }}</td>
					<td><input class="txt" type="text" size="20" value="{{ $shop->viewnum }}" name="viewnum"> 次<span class="tdtip">设置显示浏览的次数，以后在此基础上累加</span></td>
				</tr>
				</tbody>
				<tbody class="tb-body hidden">
				<tr>
					<td width="150" align="right">{{ trans('admin.brand.shop.message') }}</td>
					<td><textarea class="textarea" name="message" id="message" style="width:100%;height:400px">{{ $shop->message }}</textarea></td>
				</tr>
				</tbody>
				<tbody class="tb-body hidden">
				<tr>
					...
					<td>
						<label class="radio" for="offline_1">
							<input id="offline_1" type="radio" name="offline" value="1" {{ $shop->offline ? 'checked' : '' }}> {{ trans('admin.brand.shop.offline_1') }}
						</label>
						<label class="radio" for="offline_0">
							<input id="offline_0" type="radio" name="offline" value="0" {{ $shop->offline ? '' : 'checked' }}> {{ trans('admin.brand.shop.offline_0') }}
						</label>
					</td>
				</tr>
				<tr>
					...
					<td>
						<label class="radio" for="appoint_1">
							<input id="appoint_1" type="radio" name="appoint" value="1" {{ $shop->appoint ? 'checked' : '' }}> {{ trans('admin.brand.shop.appoint_1') }}
						</label>
						<label class="radio" for="appoint_0">
							<input id="appoint_0" type="radio" name="appoint" value="0" {{ $shop->appoint ? '' : 'checked' }}> {{ trans('admin.brand.shop.appoint_0') }}
						</label>
					</td>
				</tr>
				<tr>
					...
					<td>
						<label class="radio" for="ordermeal_1">
							<input id="ordermeal_1" type="radio" name="ordermeal" value="1" {{ $shop->ordermeal ? 'checked' : '' }}> {{ trans('admin.brand.shop.ordermeal_1') }}
						</label>
						<label class="radio" for="ordermeal_0">
							<input id="ordermeal_0" type="radio" name="ordermeal" value="0" {{ $shop->ordermeal ? '' : 'checked' }}> {{ trans('admin.brand.shop.ordermeal_0') }}
						</label>
					</td>
				</tr>
				<tr>
					...
					<td>
						<label class="radio" for="sellcard_1">
							<input id="sellcard_1" type="radio" name="sellcard" value="1" {{ $shop->sellcard ? 'checked' : '' }}> {{ trans('admin.brand.shop.sellcard_1') }}
						</label>
						<label class="radio" for="sellcard_0">
							<input id="sellcard_0" type="radio" name="sellcard" value="0" {{ $shop->sellcard ? '' : 'checked' }}> {{ trans('admin.brand.shop.sellcard_0') }}
						</label>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input class="txt" type="text" size="50" value="{{ $shop->seo_title }}" name="seo_title"></td>
				</tr>
				<tr>
					<td align="right">seo_keywords</td>
					<td><input class="txt" type="text" size="50" value="{{ $shop->seo_keywords }}" name="seo_keywords"></td>
				</tr>
				<tr>
					<td align="right">seo_description</td>
					<td><textarea class="textarea" name="seo_description" cols="60" rows="5">{{ $shop->seo_description }}</textarea></td>
				</tr>
				</tbody>
				<tr>
					<td align="right"></td>
