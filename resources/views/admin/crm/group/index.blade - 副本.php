...
@section('content')
	...
	<form id="cpform" name="cpform" class="ajaxform" method="post" action="{{ route('admin.crm.group.batch') }}">
	{!! csrf_field() !!}
	<input type="hidden" id="operate" name="operate" value="" />
	<div class="tblist">
		<div class="tbhead cl">
			<div class="z"><h3>{{ trans('admin.crm.group.list') }}</h3></div>
			<div class="y"><a href="{{ route('admin.crm.group.create') }}" class="btn openwindow" title="{{ trans('admin.crm.group.create') }}">+ {{ trans('admin.crm.group.create') }}</a></div>
		</div>
		<table>
			<tr>
			...
			<tr>
				...
				<td>
					<a href="{{ route('admin.crm.group.edit',$group->id) }}" class="openwindow" title="{{ trans('admin.crm.group.edit') }}">{{ trans('admin.edit') }}</a>
					<a href="{{ route('admin.crm.group.destroy',$group->id) }}" class="mlm delbtn">{{ trans('admin.destroy') }}</a>
				</td>
			</tr>
			@endforeach
		</table>
