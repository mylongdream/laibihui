    ...
    <div class="nav">
        <ul class="">
            @foreach ($topmenu as $tmenu)
                <li class="{{ $tmenu->current  ? 'a' : '' }}"><a href="{{ $tmenu->route ? route($tmenu->route) : '' }}">{{ $tmenu->title }}</a></li>
            @endforeach
        </ul>
    </div>
    <div class="info">
        ...
        <div id="menuBar" class="menu_box">
            ...
            <dl class="menu">
                ...
                @else
                    ...
                    <dd class="menu_item {{ $smenu->current  ? 'a' : '' }}"><a href="{{ $smenu->route ? route($smenu->route) : '' }}">{{ $smenu->title }}</a></dd>
                    @endforeach
                @endif
            </dl>
            @endforeach
...
@if (!request()->ajax())
    ...
    @if (count($errors) > 0)
        <script type="text/javascript">$.jBox.error("{{ $errors->first() }}", "提示");</script>
    @endif
</body>
</html>
@endif