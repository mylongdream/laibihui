			...
			<form class="ajaxform" enctype="multipart/form-data" method="post" action="{{ route('user.profile.update') }}">
				...
				<table>
					...
					<tr>
						...
						<td><input id="birthday" class="input" type="text" size="50" value="{{$profile->birthday ? $profile->birthday->format('Y-m-d') : ''}}" name="birthday" onclick="laydate({max: laydate.now(-1),format:'YYYY-MM-DD'})"></td>
					</tr>
					<tr>
