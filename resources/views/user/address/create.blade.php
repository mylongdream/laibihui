...
@section('content')
	...
	<div class="itemnav">
		...
		<ul class="tab">
			...
			<li class="on"><a href="{{ route('user.address.create') }}"><span>{{ trans('user.address.create') }}</span></a></li>
		</ul>
	</div>
	<div class="mtw">
		<div class="tbhead cl">
			<div class="z"><h3>{{ trans('user.address.create') }}</h3></div>
		</div>
		<div class="tbedit">
			<form class="ajaxform" enctype="multipart/form-data" method="post" action="{{ route('user.address.store') }}">
				{!! csrf_field() !!}
				<table>
					<tr>
					...
					<tr>
						...
						<td><input class="input" type="text" size="50" value="" name="zipcode"><span class="tdtip">如您不清楚邮递区号可不填</span></td>
					</tr>
					<tr>
						<td align="right"></td>
	...
	@else
		<div class="pop-form">
			<form class="addressform" enctype="multipart/form-data" method="post" action="{{ route('user.address.store') }}">
				{!! csrf_field() !!}
				<table>
					<tr>
					...
					<tr>
						<td align="right">{{ trans('user.address.zipcode') }}：</td>
						<td><input class="input" type="text" size="50" value="" name="zipcode" placeholder="如您不清楚邮递区号可不填"></td>
					</tr>
					<tr>
						<td align="right"></td>
						<td><label class="checkbox" for="default"><input id="default" name="default" value="1" type="checkbox">设置为默认收货地址</label></td>
