...
@section('content')
	<div class="itemnav">
		...
		<ul class="tab">
			...
			<li class="on"><a href="{{ route('user.promotion.first') }}"><span>{{ trans('user.promotion.first') }}</span></a></li>
			<li><a href="{{ route('user.promotion.second') }}"><span>{{ trans('user.promotion.second') }}</span></a></li>
		</ul>
	</div>
	<form id="schform" name="schform" class="formsearch" method="get" action="{{ route('user.promotion.first') }}">
		<div class="tbsearch">
			<dl>
				<dd>
