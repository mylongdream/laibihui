		...
		<div class="crm-list mtw">
			<table>
				...
				@foreach ($orders as $value)
					<tr>
						<td><a href="{{ route('crm.ordermeal.show', $value->order_sn) }}" title="查看订单" class="openwindow">{{ $value->order_sn }}</a></td>
						...
						<td>
                            ...
                            @else
                                <a href="{{ route('crm.ordermeal.edit', $value->order_sn) }}" title="处理订单" class="openwindow">{{ trans('user.appoint.status_'.$value->status) }}</a>
                            @endif
						</td>
