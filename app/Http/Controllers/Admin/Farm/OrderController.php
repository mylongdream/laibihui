...
class OrderController extends Controller
{
    ...
    {
        $FarmOrderModel = new FarmOrderModel;
        $orders = $FarmOrderModel->where(function($query) use($request) {
        ...
        return view('admin.farm.order.index', ['orders' => $orders]);
    }
    ...
    {
        $order = FarmOrderModel::findOrFail($id);
        return view('admin.farm.order.show')->with('order', $order);
    }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $order->delete();
        if ($request->ajax()){
            return response()->json(['status' => '1', 'info' => trans('admin.farm.order.deletesucceed'), 'url' => back()->getTargetUrl()]);
        }else{
            return view('admin.layouts.message', ['status' => '1', 'info' => trans('admin.farm.order.deletesucceed'), 'url' => back()->getTargetUrl()]);
        }
    }
    ...
    public function batch(Request $request)
    {
        if($request->operate == 'delsubmit') {
            ...
            FarmOrderModel::destroy($request->ids);
            if ($request->ajax()) {
                return response()->json(['status' => '1', 'info' => trans('admin.farm.order.deletesucceed'), 'url' => back()->getTargetUrl()]);
            }else{
                return view('admin.layouts.message', ['status' => '1', 'info' => trans('admin.farm.order.deletesucceed'), 'url' => back()->getTargetUrl()]);
    ...
    public function send(Request $request, $id)
    {
        ...
        if($request->isMethod('POST')){
                ...
                $send = new FarmOrderShippingModel;
            ...
            return view('admin.farm.order.send', ['order' => $order, 'shippings' => $shippings]);
        }
    }
//退款
    public function refund(Request $request, $id)
    {
        ...
        if($request->isMethod('POST')){
            ...
            return view('admin.farm.order.refund', ['order' => $order]);
        }
    }
//关闭
    public function close(Request $request, $id)
    {
        ...
        if($request->isMethod('POST')){
            ...
            return view('admin.farm.order.close', ['order' => $order]);
        }
    }
