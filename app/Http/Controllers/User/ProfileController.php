...
class ProfileController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->ajax()){
            return response()->json(['status' => '1', 'info' => trans('user.profile.addsucceed'), 'url' => route('user.profile.index')]);
        }else{
            return view('layouts.user.message', ['status' => '1', 'info' => trans('user.profile.addsucceed'), 'url' => route('user.profile.index')]);
        }
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->ajax()){
            return response()->json(['status' => '1', 'info' => trans('user.profile.updatesucceed'), 'url' => route('user.profile.index')]);
        }else{
            return view('layouts.user.message', ['status' => '1', 'info' => trans('user.profile.updatesucceed'), 'url' => route('user.profile.index')]);
        }
    }
    ...
    public function mobile(Request $request)
    {
        if ($request->isMethod('POST')) {
            ...
            if(auth()->user()->mobile){
                ...
                auth()->user()->save();
                if ($request->ajax()){
                    return response()->json(['status' => '1', 'info' => '手机号修改成功', 'url' => route('user.index')]);
                }else{
                    return view('layouts.mobile.message', ['status' => '1', 'info' => '手机号修改成功', 'url' => route('user.index')]);
                }
            }else{
                auth()->user()->mobile = $request->mobile;
                auth()->user()->save();
                if ($request->ajax()){
                    return response()->json(['status' => '1', 'info' => '手机号绑定成功', 'url' => route('user.index')]);
                }else{
                    return view('layouts.mobile.message', ['status' => '1', 'info' => '手机号绑定成功', 'url' => route('user.index')]);
                }
            }
        }else{
            return view('user.profile.mobile');
        }
    }
