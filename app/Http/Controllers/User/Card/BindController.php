...
class BindController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->isMethod('POST')){
            ...
            if ($request->ajax()){
                return response()->json(['status' => 1, 'info' => trans('user.card.bind.updatesucceed'), 'url' => route('user.card.bind.index')]);
            }else{
                return view('layouts.user.message', ['status' => 1, 'info' => trans('user.card.bind.updatesucceed'), 'url' => route('user.card.bind.index')]);
            }
        }else{
            $card = auth()->user()->card;
            return view('user.card.bind.index', ['card' => $card]);
        }
    }
}
