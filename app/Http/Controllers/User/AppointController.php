...
class AppointController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('user.appoint.index', ['appoints' => $appoints]);
    }

    public function show(Request $request, $id)
    {
        $appoint = BrandAppointModel::where('uid', auth()->user()->uid)->where('order_sn', $id)->firstOrFail();
        return view('user.appoint.show', ['appoint' => $appoint]);
    }
    ...
    public function cancel(Request $request, $id)
    {
        ...
        if($request->isMethod('POST')){
            if($appoint->status == 0){
                ...
                $appoint->save();
                if ($request->ajax()){
                    return response()->json(['status' => 1, 'info' => '取消预约成功', 'url' => back()->getTargetUrl()]);
                }else{
                    return view('layouts.common.message', ['status' => 1, 'info' => '取消预约成功', 'url' => back()->getTargetUrl()]);
                }
            }else{
                if ($request->ajax()){
                    return response()->json(['status' => 0, 'info' => '取消预约失败', 'url' => back()->getTargetUrl()]);
                }else{
                    return view('layouts.common.message', ['status' => 0, 'info' => '取消预约失败', 'url' => back()->getTargetUrl()]);
                }
            }
        }else{
            return view('user.appoint.cancel', ['appoint' => $appoint]);
        }
    }
