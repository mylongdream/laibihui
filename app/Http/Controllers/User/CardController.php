...
class CardController extends Controller
{
    ...
    public function appoint(Request $request)
    {
        ...
        return view('user.card.appoint', ['appoints' => $appoints]);
    }
    ...
    public function bind(Request $request)
    {
        ...
        if($request->isMethod('POST')){
            ...
            if ($request->ajax()){
                return response()->json(['status' => 1, 'info' => trans('user.card.bind.updatesucceed'), 'url' => route('user.card.bind')]);
            }else{
                return view('layouts.user.message', ['status' => 1, 'info' => trans('user.card.bind.updatesucceed'), 'url' => route('user.card.bind')]);
            }
        }else{
            $card = auth()->user()->card;
            return view('user.card.bind', ['card' => $card]);
        }
    }
}
