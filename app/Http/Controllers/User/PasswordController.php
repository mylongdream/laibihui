...
class PasswordController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->ajax()){
            return response()->json(['status' => '1', 'info' => trans('user.password.editsucceed'), 'url' => route('user.password.index')]);
        }else{
            return view('layouts.user.message', ['status' => '1', 'info' => trans('user.password.editsucceed'), 'url' => route('user.password.index')]);
        }
    }
}
