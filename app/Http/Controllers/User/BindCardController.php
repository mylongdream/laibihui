...
class BindCardController extends Controller
{
    ...
    public function index(Request $request)
    {
        if($request->isMethod('POST')){
            ...
            if(!$card){
                if ($request->ajax()){
                    return response()->json(['status' => 0, 'info' => trans('user.bindcard.passwordwrong')]);
                }else{
            ...
            if ($request->ajax()){
                return response()->json(['status' => 1, 'info' => trans('user.bindcard.updatesucceed'), 'url' => route('user.bindcard.index')]);
            }else{
                return view('layouts.user.message', ['status' => 1, 'info' => trans('user.bindcard.updatesucceed'), 'url' => route('user.bindcard.index')]);
            }
        }else{
            $card = auth()->user()->card;
            return view('user.bindcard.index', ['card' => $card]);
        }
    }
}
