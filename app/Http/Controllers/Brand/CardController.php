...
class CardController extends Controller
{
        ...
        if ($request->isMethod('POST')) {
            ...
            if($forbid && $request->ordertype == 0){
                if ($request->ajax()){
                    ...
                    return view('layouts.common.message', ['status' => 0, 'info' => '所在地区不支持上门办卡']);
                }
            }
            ...
            if ($request->ajax()) {
                return response()->json(['status' => 1, 'info' => '', 'url' => route('brand.card.pay', ['id' => $order->order_sn])]);
            } else {
                return view('layouts.common.message', ['status' => 1, 'info' => '', 'url' => route('brand.card.pay', ['id' => $order->order_sn])]);
            }
        } else {
            return view('brand.card.order');
        }
    }
    ...
    public function addresslist(Request $request){
        return view('brand.card.address');
    }

    public function pay(Request $request, $id){
        $setting = cache('setting');
        $order = CommonCardOrderModel::where('uid', auth()->user()->uid)->where('order_sn', $id)->first();
        if ($request->isMethod('POST')) {
            ...
            if ($request->ajax()) {
                ...
                return view('layouts.common.message', ['status' => 1, 'info' => '预约办卡成功', 'url' => route('user.card.appoint')]);
            }
        } else {
            return view('brand.card.pay', ['order' => $order]);
        }
    }
        ...
        if($request->isMethod('POST')){
            ...
            if(!auth()->check()){
                ...
                if (!$user){ //自动注册
                    event(new Registered($user = CommonUserModel::create([
                        ...
                        'password' => bcrypt(substr($request->mobile, -6)),
                        'mobile' => $request->mobile,
                        'regip' => request()->getClientIp(),
                    ])));
                    auth()->guard()->login($user);

                    if ($fromuid && $fromuid != auth()->user()->uid && isset($setting['promotion_register']) && $setting['promotion_register']){
                        $fromuser = CommonUserModel::where('uid', $fromuid)->first();
                        if ($fromuser) {
                    ...
                    $content = '您刚办卡时已自动注册成为会员，您的登录密码是：'.substr($request->mobile, -6);
            ...
            if(auth()->user()->card){
                if ($request->ajax()){
                    ...
                    return view('layouts.common.message', ['status' => 0, 'info' => '您已开卡，无需再办卡']);
                }
            }
            ...
            return view('brand.card.appoint');
        }
    }

    public function active(Request $request){
        if(auth()->check()){
            return redirect(route('user.bindcard.index'));
        }
        if($request->isMethod('POST')){
            $rules = array(
            ...
            if ($request->ajax()){
                return response()->json(['status' => 1, 'info' => '在线开卡成功', 'url' => route('user.index')]);
            }else{
                return view('layouts.common.message', ['status' => 1, 'info' => '在线开卡成功', 'url' => route('user.index')]);
            }
        }else{
            return view('brand.card.active');
        }
    }
