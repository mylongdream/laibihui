...
class ScoreController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('mobile.user.score.index', ['scores' => $scores]);
    }
        ...
        if($request->isMethod('POST')){
            ...
            if($amount > auth()->user()->score){
                if ($request->ajax()){
                    ...
                    return view('layouts.mobile.message', ['status' => 0, 'info' => '转让积分数不足', 'url' => back()->getTargetUrl()]);
                }
            }
            ...
            if(!$user){
                if ($request->ajax()){
                    ...
                    return view('layouts.mobile.message', ['status' => 0, 'info' => '转让对方账户不存在', 'url' => back()->getTargetUrl()]);
                }
            }
            ...
            if ($request->ajax()){
                ...
                return view('layouts.mobile.message', ['status' => 1, 'info' => trans('user.score.transfersucceed'), 'url' => back()->getTargetUrl()]);
            }
        }else{
            return view('mobile.user.score.transfer');
        }
    }
